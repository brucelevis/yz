data_0201_Skill = {
	[100001] = {
		name = "物理攻击",
		des = "普通物理攻击(占位用，实现逻辑单独处理)",
		skillPre = 0,
		skillPreLv = 0,
		skillPrePt = 0,
		maxLv = 0,
		attackType = 2,
		dstType = 0,
		singTime = 0,
		minSingTime = 0,
		cdTime = 0,
		skilleffect = {},
		buff = {},
		jobID = 0,
	},

	[100011] = {
		name = "英勇意志",
		des = "增加999点生命值",
		skillPre = 0,
		skillPreLv = 0,
		skillPrePt = 0,
		maxLv = 5,
		attackType = 1,
		dstType = 0,
		singTime = 0,
		minSingTime = 0,
		cdTime = 0,
		skilleffect = {900001},
		buff = {},
		jobID = 10001,
	},

	[100012] = {
		name = "重击",
		des = "对敌人单体造成999%攻击力+999点物理伤害",
		skillPre = 0,
		skillPreLv = 0,
		skillPrePt = 0,
		maxLv = 1,
		attackType = 2,
		dstType = 0,
		singTime = 2.8,
		minSingTime = 1,
		cdTime = 1.5,
		skilleffect = {900002.0},
		buff = {},
		jobID = 10001,
	},

	[100013] = {
		name = "火焰冲击",
		des = "对敌人单体造成999%法术强度+999点法术伤害",
		skillPre = 0,
		skillPreLv = 0,
		skillPrePt = 0,
		maxLv = 1,
		attackType = 2,
		dstType = 0,
		singTime = 2.8,
		minSingTime = 1,
		cdTime = 1.5,
		skilleffect = {900002.0},
		buff = {},
		jobID = 10001,
	},

	[100014] = {
		name = "急救",
		des = "对自己回复999%法术强度+999点生命值",
		skillPre = 0,
		skillPreLv = 0,
		skillPrePt = 0,
		maxLv = 1,
		attackType = 2,
		dstType = 0,
		singTime = 2.8,
		minSingTime = 1,
		cdTime = 1.5,
		skilleffect = {900002.0},
		buff = {},
		jobID = 10001,
	},

	[101000] = {
		name = "单手剑修炼",
		des = "使用单手剑时增加999攻击力",
		skillPre = 0,
		skillPreLv = 0,
		skillPrePt = 0,
		maxLv = 1,
		attackType = 2,
		dstType = 0,
		singTime = 2.8,
		minSingTime = 1,
		cdTime = 1.5,
		skilleffect = {900002.0},
		buff = {},
		jobID = 10002,
	},

	[101010] = {
		name = "双手剑修炼",
		des = "使用双手剑时增加999攻击力",
		skillPre = 0,
		skillPreLv = 0,
		skillPrePt = 0,
		maxLv = 1,
		attackType = 2,
		dstType = 0,
		singTime = 2.8,
		minSingTime = 1,
		cdTime = 1.5,
		skilleffect = {900002.0},
		buff = {},
		jobID = 10002,
	},

	[101020] = {
		name = "快速回复",
		des = "受到治疗的效果增加999%",
		skillPre = 0,
		skillPreLv = 0,
		skillPrePt = 0,
		maxLv = 1,
		attackType = 2,
		dstType = 0,
		singTime = 2.8,
		minSingTime = 1,
		cdTime = 1.5,
		skilleffect = {900002.0},
		buff = {},
		jobID = 10002,
	},

	[101030] = {
		name = "狂击",
		des = "对敌人单体造成999%攻击力+999点物理伤害,并有999%概率使敌人陷入昏迷状态",
		skillPre = 0,
		skillPreLv = 0,
		skillPrePt = 0,
		maxLv = 1,
		attackType = 2,
		dstType = 0,
		singTime = 2.8,
		minSingTime = 1,
		cdTime = 1.5,
		skilleffect = {900002.0},
		buff = {},
		jobID = 10002,
	},

	[101040] = {
		name = "怒爆",
		des = "对范围敌人造成999%攻击力+999点火属性伤害,并有999%概率击退敌人",
		skillPre = 0,
		skillPreLv = 0,
		skillPrePt = 0,
		maxLv = 1,
		attackType = 2,
		dstType = 0,
		singTime = 2.8,
		minSingTime = 1,
		cdTime = 1.5,
		skilleffect = {900002.0},
		buff = {},
		jobID = 10002,
	},

	[101050] = {
		name = "挑衅",
		des = "使敌人强制攻击自己,敌人防御力降低999点,攻击力提高999点.对不死族无效",
		skillPre = 0,
		skillPreLv = 0,
		skillPrePt = 0,
		maxLv = 1,
		attackType = 2,
		dstType = 0,
		singTime = 2.8,
		minSingTime = 1,
		cdTime = 1.5,
		skilleffect = {900002.0},
		buff = {},
		jobID = 10002,
	},

	[101060] = {
		name = "盾墙",
		des = "减少受到的40%近战物理伤害,并增加自身999点防御,持续999秒.装备盾牌时才能使用.",
		skillPre = 0,
		skillPreLv = 0,
		skillPrePt = 0,
		maxLv = 1,
		attackType = 2,
		dstType = 0,
		singTime = 2.8,
		minSingTime = 1,
		cdTime = 1.5,
		skilleffect = {900002.0},
		buff = {},
		jobID = 10002,
	},

	[101070] = {
		name = "霸体",
		des = "增加法术防御力999点,免疫状态变化，持续999秒",
		skillPre = 0,
		skillPreLv = 0,
		skillPrePt = 0,
		maxLv = 1,
		attackType = 2,
		dstType = 0,
		singTime = 2.8,
		minSingTime = 1,
		cdTime = 1.5,
		skilleffect = {900002.0},
		buff = {},
		jobID = 10002,
	},

	[102000] = {
		name = "天使之力",
		des = "增加自身面对魔鬼系和不死系敌人时的攻击力999点,法术强度999点，防御力999点",
		skillPre = 0,
		skillPreLv = 0,
		skillPrePt = 0,
		maxLv = 1,
		attackType = 2,
		dstType = 0,
		singTime = 2.8,
		minSingTime = 1,
		cdTime = 1.5,
		skilleffect = {900002.0},
		buff = {},
		jobID = 10003,
	},

	[102010] = {
		name = "天使之心",
		des = "使用治疗法术时，法术强度增加999点",
		skillPre = 0,
		skillPreLv = 0,
		skillPrePt = 0,
		maxLv = 1,
		attackType = 2,
		dstType = 0,
		singTime = 2.8,
		minSingTime = 1,
		cdTime = 1.5,
		skilleffect = {900002.0},
		buff = {},
		jobID = 10003,
	},

	[102020] = {
		name = "光猎",
		des = "召唤光球寻找敌方潜行单位,造成999%法术强度+999点圣属性伤害,持续999秒",
		skillPre = 0,
		skillPreLv = 0,
		skillPrePt = 0,
		maxLv = 1,
		attackType = 2,
		dstType = 0,
		singTime = 2.8,
		minSingTime = 1,
		cdTime = 1.5,
		skilleffect = {900002.0},
		buff = {},
		jobID = 10003,
	},

	[102030] = {
		name = "神圣之光",
		des = "对敌人单体造成999%法术强度+999点圣属性伤害.并且可以击破障壁.",
		skillPre = 0,
		skillPreLv = 0,
		skillPrePt = 0,
		maxLv = 1,
		attackType = 2,
		dstType = 0,
		singTime = 2.8,
		minSingTime = 1,
		cdTime = 1.5,
		skilleffect = {900002.0},
		buff = {},
		jobID = 10003,
	},

	[102040] = {
		name = " 治疗术",
		des = "回复一个友方单体999%法术强度+999点生命值",
		skillPre = 0,
		skillPreLv = 0,
		skillPrePt = 0,
		maxLv = 1,
		attackType = 2,
		dstType = 0,
		singTime = 2.8,
		minSingTime = 1,
		cdTime = 1.5,
		skilleffect = {900002.0},
		buff = {},
		jobID = 10003,
	},

	[102050] = {
		name = " 复活术",
		des = "复活一个友方单位,复活后拥有999%法术强度+999点生命值",
		skillPre = 0,
		skillPreLv = 0,
		skillPrePt = 0,
		maxLv = 1,
		attackType = 2,
		dstType = 0,
		singTime = 2.8,
		minSingTime = 1,
		cdTime = 1.5,
		skilleffect = {900002.0},
		buff = {},
		jobID = 10003,
	},

	[102060] = {
		name = "守护光环",
		des = "增加全体友方单体999点防御力,自身法力上限、法力回复降低10%.队伍内只能生效一种同类光环",
		skillPre = 0,
		skillPreLv = 0,
		skillPrePt = 0,
		maxLv = 1,
		attackType = 2,
		dstType = 0,
		singTime = 2.8,
		minSingTime = 1,
		cdTime = 1.5,
		skilleffect = {900002.0},
		buff = {},
		jobID = 10003,
	},

	[102070] = {
		name = "力量光环",
		des = "增加全体友方单体999点攻击力与法术强度,自身法力上限、法力回复降低10%.队伍内只能生效一种同类光环",
		skillPre = 0,
		skillPreLv = 0,
		skillPrePt = 0,
		maxLv = 1,
		attackType = 2,
		dstType = 0,
		singTime = 2.8,
		minSingTime = 1,
		cdTime = 1.5,
		skilleffect = {900002.0},
		buff = {},
		jobID = 10003,
	},

	[102080] = {
		name = "速度光环",
		des = "增加全体友方单体999点攻击速度,自身法力上限、法力回复降低10%.队伍内只能生效一种同类光环",
		skillPre = 0,
		skillPreLv = 0,
		skillPrePt = 0,
		maxLv = 1,
		attackType = 2,
		dstType = 0,
		singTime = 2.8,
		minSingTime = 1,
		cdTime = 1.5,
		skilleffect = {900002.0},
		buff = {},
		jobID = 10003,
	},

	[102090] = {
		name = "缓速术",
		des = "降低一个敌方单体999点攻击速度，持续999秒",
		skillPre = 0,
		skillPreLv = 0,
		skillPrePt = 0,
		maxLv = 1,
		attackType = 2,
		dstType = 0,
		singTime = 2.8,
		minSingTime = 1,
		cdTime = 1.5,
		skilleffect = {900002.0},
		buff = {},
		jobID = 10003,
	},

	[102100] = {
		name = "光之障壁",
		des = "给友方单体施放一面光墙,免疫远程物理攻击,持续999秒或抵挡999次伤害有失效.每个服侍在同一时间内,只能给队友施放一种障壁效果.",
		skillPre = 0,
		skillPreLv = 0,
		skillPrePt = 0,
		maxLv = 1,
		attackType = 2,
		dstType = 0,
		singTime = 2.8,
		minSingTime = 1,
		cdTime = 1.5,
		skilleffect = {900002.0},
		buff = {},
		jobID = 10003,
	},

	[103000] = {
		name = "禅心",
		des = "使用法杖增加999攻击力,并增加自身999点魔法值回复",
		skillPre = 0,
		skillPreLv = 0,
		skillPrePt = 0,
		maxLv = 1,
		attackType = 2,
		dstType = 0,
		singTime = 2.8,
		minSingTime = 1,
		cdTime = 1.5,
		skilleffect = {900002.0},
		buff = {},
		jobID = 10004,
	},

	[103010] = {
		name = "冰箭术",
		des = "对敌人单体造成999%法术强度+999点水属性伤害,并有999%概率冻结敌人.冰冻后受到雷系伤害提高",
		skillPre = 0,
		skillPreLv = 0,
		skillPrePt = 0,
		maxLv = 1,
		attackType = 2,
		dstType = 0,
		singTime = 2.8,
		minSingTime = 1,
		cdTime = 1.5,
		skilleffect = {900002.0},
		buff = {},
		jobID = 10004,
	},

	[103020] = {
		name = " 霜冻之术",
		des = "对敌人群体造成999%法术强度+999点水属性伤害,并有999%概率冻结敌人.冰冻后受到雷系伤害提高",
		skillPre = 0,
		skillPreLv = 0,
		skillPrePt = 0,
		maxLv = 1,
		attackType = 2,
		dstType = 0,
		singTime = 2.8,
		minSingTime = 1,
		cdTime = 1.5,
		skilleffect = {900002.0},
		buff = {},
		jobID = 10004,
	},

	[103030] = {
		name = "火箭术",
		des = "召唤火箭对敌人单体造成999%法术强度+999点火属性伤害",
		skillPre = 0,
		skillPreLv = 0,
		skillPrePt = 0,
		maxLv = 1,
		attackType = 2,
		dstType = 0,
		singTime = 2.8,
		minSingTime = 1,
		cdTime = 1.5,
		skilleffect = {900002.0},
		buff = {},
		jobID = 10004,
	},

	[103040] = {
		name = "火球术",
		des = "召唤火球对敌人群体造成999%法术强度+999点火属性伤害,相邻敌人受到伤害会衰减",
		skillPre = 0,
		skillPreLv = 0,
		skillPrePt = 0,
		maxLv = 1,
		attackType = 2,
		dstType = 0,
		singTime = 2.8,
		minSingTime = 1,
		cdTime = 1.5,
		skilleffect = {900002.0},
		buff = {},
		jobID = 10004,
	},

	[103050] = {
		name = "火猎",
		des = "召唤火球寻找敌方潜行单位,造成999%法术强度+999点火属性伤害,持续999秒",
		skillPre = 0,
		skillPreLv = 0,
		skillPrePt = 0,
		maxLv = 1,
		attackType = 2,
		dstType = 0,
		singTime = 2.8,
		minSingTime = 1,
		cdTime = 1.5,
		skilleffect = {900002.0},
		buff = {},
		jobID = 10004,
	},

	[103060] = {
		name = "雷击术",
		des = "召唤雷电对敌人单体造成999%法术强度+999点雷属性伤害",
		skillPre = 0,
		skillPreLv = 0,
		skillPrePt = 0,
		maxLv = 1,
		attackType = 2,
		dstType = 0,
		singTime = 2.8,
		minSingTime = 1,
		cdTime = 1.5,
		skilleffect = {900002.0},
		buff = {},
		jobID = 10004,
	},

	[103070] = {
		name = "雷爆术",
		des = "召唤雷电对敌人群体造成999%法术强度+999点雷属性伤害,相邻敌人受到伤害会衰减",
		skillPre = 0,
		skillPreLv = 0,
		skillPrePt = 0,
		maxLv = 1,
		attackType = 2,
		dstType = 0,
		singTime = 2.8,
		minSingTime = 1,
		cdTime = 1.5,
		skilleffect = {900002.0},
		buff = {},
		jobID = 10004,
	},

	[103080] = {
		name = "圣灵召唤",
		des = "使用念力对敌人单体造成999%法术强度+999点念属性伤害",
		skillPre = 0,
		skillPreLv = 0,
		skillPrePt = 0,
		maxLv = 1,
		attackType = 2,
		dstType = 0,
		singTime = 2.8,
		minSingTime = 1,
		cdTime = 1.5,
		skilleffect = {900002.0},
		buff = {},
		jobID = 10004,
	},

	[103090] = {
		name = "心灵爆破",
		des = "使用念力对敌人群体造成999%法术强度+999点念属性伤害,相邻敌人受到伤害会衰减",
		skillPre = 0,
		skillPreLv = 0,
		skillPrePt = 0,
		maxLv = 1,
		attackType = 2,
		dstType = 0,
		singTime = 2.8,
		minSingTime = 1,
		cdTime = 1.5,
		skilleffect = {900002.0},
		buff = {},
		jobID = 10004,
	},

	[103100] = {
		name = "石化术",
		des = "对敌人单体造成999%法术强度+999点土属性伤害,并有999%概率石化敌人.石化后受到火系伤害提高",
		skillPre = 0,
		skillPreLv = 0,
		skillPrePt = 0,
		maxLv = 1,
		attackType = 2,
		dstType = 0,
		singTime = 2.8,
		minSingTime = 1,
		cdTime = 1.5,
		skilleffect = {900002.0},
		buff = {},
		jobID = 10004,
	},

	[103110] = {
		name = "能量外套",
		des = "减少999%的物理及物理技能伤害,持续999秒.",
		skillPre = 0,
		skillPreLv = 0,
		skillPrePt = 0,
		maxLv = 1,
		attackType = 2,
		dstType = 0,
		singTime = 2.8,
		minSingTime = 1,
		cdTime = 1.5,
		skilleffect = {900002.0},
		buff = {},
		jobID = 10004,
	},

	[104000] = {
		name = "二刀连击",
		des = "使用短剑或拳刃时有999%概率进行一次额外攻击,造成50%攻击力的伤害",
		skillPre = 0,
		skillPreLv = 0,
		skillPrePt = 0,
		maxLv = 1,
		attackType = 2,
		dstType = 0,
		singTime = 2.8,
		minSingTime = 1,
		cdTime = 1.5,
		skilleffect = {900002.0},
		buff = {},
		jobID = 10005,
	},

	[104010] = {
		name = " 残影",
		des = "增加自身999点躲闪.",
		skillPre = 0,
		skillPreLv = 0,
		skillPrePt = 0,
		maxLv = 1,
		attackType = 2,
		dstType = 0,
		singTime = 2.8,
		minSingTime = 1,
		cdTime = 1.5,
		skilleffect = {900002.0},
		buff = {},
		jobID = 10005,
	},

	[104020] = {
		name = "施毒",
		des = "对敌人单体造成999%攻击力+999点毒伤害,并有999%概率使敌人中毒",
		skillPre = 0,
		skillPreLv = 0,
		skillPrePt = 0,
		maxLv = 1,
		attackType = 2,
		dstType = 0,
		singTime = 2.8,
		minSingTime = 1,
		cdTime = 1.5,
		skilleffect = {900002.0},
		buff = {},
		jobID = 10005,
	},

	[104030] = {
		name = "投石",
		des = "对敌人单体造成999%攻击力+999物理伤害,并有999%概率使敌人昏迷",
		skillPre = 0,
		skillPreLv = 0,
		skillPrePt = 0,
		maxLv = 1,
		attackType = 2,
		dstType = 0,
		singTime = 2.8,
		minSingTime = 1,
		cdTime = 1.5,
		skilleffect = {900002.0},
		buff = {},
		jobID = 10005,
	},

	[104040] = {
		name = "喷沙",
		des = "对敌人单体造成999%攻击力+999物理伤害,并有999%概率使敌人沉默",
		skillPre = 0,
		skillPreLv = 0,
		skillPrePt = 0,
		maxLv = 1,
		attackType = 2,
		dstType = 0,
		singTime = 2.8,
		minSingTime = 1,
		cdTime = 1.5,
		skilleffect = {900002.0},
		buff = {},
		jobID = 10005,
	},

	[104050] = {
		name = "偷窃",
		des = "随机偷取敌人身上,攻击、防御、法术强度、法术防御其中一项属性999点,持续99秒",
		skillPre = 0,
		skillPreLv = 0,
		skillPrePt = 0,
		maxLv = 1,
		attackType = 2,
		dstType = 0,
		singTime = 2.8,
		minSingTime = 1,
		cdTime = 1.5,
		skilleffect = {900002.0},
		buff = {},
		jobID = 10005,
	},

	[104060] = {
		name = "隐匿",
		des = "进入隐身状态,免疫物理伤害,不能成为技能攻击目标,生命值回复+999,但无法攻击,持续999秒.使用技能后解除.",
		skillPre = 0,
		skillPreLv = 0,
		skillPrePt = 0,
		maxLv = 1,
		attackType = 2,
		dstType = 0,
		singTime = 2.8,
		minSingTime = 1,
		cdTime = 1.5,
		skilleffect = {900002.0},
		buff = {},
		jobID = 10005,
	},

	[105000] = {
		name = " 苍鹰之眼",
		des = "使用弓箭时增加999攻击力",
		skillPre = 0,
		skillPreLv = 0,
		skillPrePt = 0,
		maxLv = 1,
		attackType = 2,
		dstType = 0,
		singTime = 2.8,
		minSingTime = 1,
		cdTime = 1.5,
		skilleffect = {900002.0},
		buff = {},
		jobID = 10007,
	},

	[105010] = {
		name = "二连矢",
		des = "连续射出两支箭,对敌人单体造成999%攻击力+999点物理伤害",
		skillPre = 0,
		skillPreLv = 0,
		skillPrePt = 0,
		maxLv = 1,
		attackType = 2,
		dstType = 0,
		singTime = 2.8,
		minSingTime = 1,
		cdTime = 1.5,
		skilleffect = {900002.0},
		buff = {},
		jobID = 10007,
	},

	[105020] = {
		name = " 箭雨",
		des = "连续射出多支箭,每支箭对敌人造成999%攻击力+999点物理伤害,攻击同一个单位时伤害会下降",
		skillPre = 0,
		skillPreLv = 0,
		skillPrePt = 0,
		maxLv = 1,
		attackType = 2,
		dstType = 0,
		singTime = 2.8,
		minSingTime = 1,
		cdTime = 1.5,
		skilleffect = {900002.0},
		buff = {},
		jobID = 10007,
	},

	[105030] = {
		name = " 冲锋箭",
		des = "对敌人单体造成999%攻击力+999点物理伤害,并有999%概率击退敌人",
		skillPre = 0,
		skillPreLv = 0,
		skillPrePt = 0,
		maxLv = 1,
		attackType = 2,
		dstType = 0,
		singTime = 2.8,
		minSingTime = 1,
		cdTime = 1.5,
		skilleffect = {900002.0},
		buff = {},
		jobID = 10007,
	},

	[105040] = {
		name = "定身箭",
		des = "对敌人单体造成999%攻击力+999点物理伤害,并有999%概率使敌人定身",
		skillPre = 0,
		skillPreLv = 0,
		skillPrePt = 0,
		maxLv = 1,
		attackType = 2,
		dstType = 0,
		singTime = 2.8,
		minSingTime = 1,
		cdTime = 1.5,
		skilleffect = {900002.0},
		buff = {},
		jobID = 10007,
	},

	[105050] = {
		name = "心神凝聚",
		des = "增加自身999点躲闪与命中,持续999秒",
		skillPre = 0,
		skillPreLv = 0,
		skillPrePt = 0,
		maxLv = 1,
		attackType = 2,
		dstType = 0,
		singTime = 2.8,
		minSingTime = 1,
		cdTime = 1.5,
		skilleffect = {900002.0},
		buff = {},
		jobID = 10007,
	},

	[106000] = {
		name = "长矛使修炼",
		des = "使用长矛时增加999攻击力",
		skillPre = 0,
		skillPreLv = 0,
		skillPrePt = 0,
		maxLv = 1,
		attackType = 2,
		dstType = 0,
		singTime = 2.8,
		minSingTime = 1,
		cdTime = 1.5,
		skilleffect = {900002.0},
		buff = {},
		jobID = 10008,
	},

	[106010] = {
		name = "乘骑术",
		des = "允许乘骑大嘴鸟,对大型魔物伤害+999%",
		skillPre = 0,
		skillPreLv = 0,
		skillPrePt = 0,
		maxLv = 1,
		attackType = 2,
		dstType = 0,
		singTime = 2.8,
		minSingTime = 1,
		cdTime = 1.5,
		skilleffect = {900002.0},
		buff = {},
		jobID = 10008,
	},

	[106020] = {
		name = "骑乘攻击",
		des = "对范围敌人造成999%攻击力+999点物理伤害,乘骑大嘴鸟,且装备长矛时才能使用.",
		skillPre = 0,
		skillPreLv = 0,
		skillPrePt = 0,
		maxLv = 1,
		attackType = 2,
		dstType = 0,
		singTime = 2.8,
		minSingTime = 1,
		cdTime = 1.5,
		skilleffect = {900002.0},
		buff = {},
		jobID = 10008,
	},

	[106030] = {
		name = "连刺攻击",
		des = "连续攻击敌人,敌人体型越大连击次数越多.每次攻击对敌人单体造成999%攻击力+999点物理伤害,装备长矛时才能使用.",
		skillPre = 0,
		skillPreLv = 0,
		skillPrePt = 0,
		maxLv = 1,
		attackType = 2,
		dstType = 0,
		singTime = 2.8,
		minSingTime = 1,
		cdTime = 1.5,
		skilleffect = {900002.0},
		buff = {},
		jobID = 10008,
	},

	[106040] = {
		name = "投掷长矛",
		des = "对敌人单体造成999%攻击力+999点远程物理伤害,备长矛时才能使用.",
		skillPre = 0,
		skillPreLv = 0,
		skillPrePt = 0,
		maxLv = 1,
		attackType = 2,
		dstType = 0,
		singTime = 2.8,
		minSingTime = 1,
		cdTime = 1.5,
		skilleffect = {900002.0},
		buff = {},
		jobID = 10008,
	},

	[106050] = {
		name = "冲击",
		des = "对敌人单体造成999%攻击力+999点物理伤害,并有999%概率击退敌人",
		skillPre = 0,
		skillPreLv = 0,
		skillPrePt = 0,
		maxLv = 1,
		attackType = 2,
		dstType = 0,
		singTime = 2.8,
		minSingTime = 1,
		cdTime = 1.5,
		skilleffect = {900002.0},
		buff = {},
		jobID = 10008,
	},

	[106060] = {
		name = " 怪物互击",
		des = "使敌人与敌人互相撞击,对敌人单体造成999%攻击力+999点物理伤害.装备单手剑或双手剑时才能使用.",
		skillPre = 0,
		skillPreLv = 0,
		skillPrePt = 0,
		maxLv = 1,
		attackType = 2,
		dstType = 0,
		singTime = 2.8,
		minSingTime = 1,
		cdTime = 1.5,
		skilleffect = {900002.0},
		buff = {},
		jobID = 10008,
	},

	[106070] = {
		name = "双手剑加速",
		des = "使用双手剑时攻击速度+999,持续999秒",
		skillPre = 0,
		skillPreLv = 0,
		skillPrePt = 0,
		maxLv = 1,
		attackType = 2,
		dstType = 0,
		singTime = 2.8,
		minSingTime = 1,
		cdTime = 1.5,
		skilleffect = {900002.0},
		buff = {},
		jobID = 10008,
	},

	[106080] = {
		name = "反击",
		des = "受到下一次近战物理攻击时进行反击,增加+999点物理伤害.必须装备双手剑才能使用.",
		skillPre = 0,
		skillPreLv = 0,
		skillPrePt = 0,
		maxLv = 1,
		attackType = 2,
		dstType = 0,
		singTime = 2.8,
		minSingTime = 1,
		cdTime = 1.5,
		skilleffect = {900002.0},
		buff = {},
		jobID = 10008,
	},

	[107000] = {
		name = "锤修炼",
		des = "使用锤时增加999攻击力",
		skillPre = 0,
		skillPreLv = 0,
		skillPrePt = 0,
		maxLv = 1,
		attackType = 2,
		dstType = 0,
		singTime = 2.8,
		minSingTime = 1,
		cdTime = 1.5,
		skilleffect = {900002.0},
		buff = {},
		jobID = 10010,
	},

	[107010] = {
		name = "沉默之术",
		des = "有999%概率使敌人沉默,持续999秒",
		skillPre = 0,
		skillPreLv = 0,
		skillPrePt = 0,
		maxLv = 1,
		attackType = 2,
		dstType = 0,
		singTime = 2.8,
		minSingTime = 1,
		cdTime = 1.5,
		skilleffect = {900002.0},
		buff = {},
		jobID = 10010,
	},

	[107020] = {
		name = "转生术",
		des = "对不死系敌人造成999%法术强度+999点，并有999%概率一击消灭不死系敌人",
		skillPre = 0,
		skillPreLv = 0,
		skillPrePt = 0,
		maxLv = 1,
		attackType = 2,
		dstType = 0,
		singTime = 2.8,
		minSingTime = 1,
		cdTime = 1.5,
		skilleffect = {900002.0},
		buff = {},
		jobID = 10010,
	},

	[107030] = {
		name = "十字驱魔",
		des = "对全体不死系、恶魔系敌人造成999%法术强度+999点神圣性伤害",
		skillPre = 0,
		skillPreLv = 0,
		skillPrePt = 0,
		maxLv = 1,
		attackType = 2,
		dstType = 0,
		singTime = 2.8,
		minSingTime = 1,
		cdTime = 1.5,
		skilleffect = {900002.0},
		buff = {},
		jobID = 10010,
	},

	[107040] = {
		name = "天使之怒",
		des = "使敌人受到的下一次技能伤害增加999%",
		skillPre = 0,
		skillPreLv = 0,
		skillPrePt = 0,
		maxLv = 1,
		attackType = 2,
		dstType = 0,
		singTime = 2.8,
		minSingTime = 1,
		cdTime = 1.5,
		skilleffect = {900002.0},
		buff = {},
		jobID = 10010,
	},

	[107050] = {
		name = "撒水祈福",
		des = "使一个友方单位的武器变为圣属性，攻击力增加999,持续999秒.",
		skillPre = 0,
		skillPreLv = 0,
		skillPrePt = 0,
		maxLv = 1,
		attackType = 2,
		dstType = 0,
		singTime = 2.8,
		minSingTime = 1,
		cdTime = 1.5,
		skilleffect = {900002.0},
		buff = {},
		jobID = 10010,
	},

	[107060] = {
		name = "幸运光环",
		des = "增加全体友方单体999点躲闪,999点暴击,持续999秒,自身法力上限、法力回复降低10%.队伍内只能生效一种同类光环",
		skillPre = 0,
		skillPreLv = 0,
		skillPrePt = 0,
		maxLv = 1,
		attackType = 2,
		dstType = 0,
		singTime = 2.8,
		minSingTime = 1,
		cdTime = 1.5,
		skilleffect = {900002.0},
		buff = {},
		jobID = 10010,
	},

	[107070] = {
		name = "暗之障壁",
		des = "给友方单体施放一面念力墙,免疫近战物理攻击,持续999秒或抵挡999次伤害有失效.每个服侍在同一时间内,只能给队友施放一种障壁效果.",
		skillPre = 0,
		skillPreLv = 0,
		skillPrePt = 0,
		maxLv = 1,
		attackType = 2,
		dstType = 0,
		singTime = 2.8,
		minSingTime = 1,
		cdTime = 1.5,
		skilleffect = {900002.0},
		buff = {},
		jobID = 10010,
	},

	[107080] = {
		name = "霸邪障壁",
		des = "给友方单体施放一面保护膜,吸收999%法术+999点伤害,持续999秒.每个服侍在同一时间内,只能给队友施放一种障壁效果.",
		skillPre = 0,
		skillPreLv = 0,
		skillPrePt = 0,
		maxLv = 1,
		attackType = 2,
		dstType = 0,
		singTime = 2.8,
		minSingTime = 1,
		cdTime = 1.5,
		skilleffect = {900002.0},
		buff = {},
		jobID = 10010,
	},

	[107090] = {
		name = " 群体治疗",
		des = "释放光芒回复群体单位999%法术强度+999点生命值",
		skillPre = 0,
		skillPreLv = 0,
		skillPrePt = 0,
		maxLv = 1,
		attackType = 2,
		dstType = 0,
		singTime = 2.8,
		minSingTime = 1,
		cdTime = 1.5,
		skilleffect = {900002.0},
		buff = {},
		jobID = 10010,
	},

	[107100] = {
		name = "净化",
		des = "清除一个友方单位的负面魔法效果,等级越高能清除的效果越多",
		skillPre = 0,
		skillPreLv = 0,
		skillPrePt = 0,
		maxLv = 1,
		attackType = 2,
		dstType = 0,
		singTime = 2.8,
		minSingTime = 1,
		cdTime = 1.5,
		skilleffect = {900002.0},
		buff = {},
		jobID = 10010,
	},

	[108000] = {
		name = "咒文加速",
		des = "增加999点咏唱速度",
		skillPre = 0,
		skillPreLv = 0,
		skillPrePt = 0,
		maxLv = 1,
		attackType = 2,
		dstType = 0,
		singTime = 2.8,
		minSingTime = 1,
		cdTime = 1.5,
		skilleffect = {900002.0},
		buff = {},
		jobID = 10012,
	},

	[108010] = {
		name = " 地震术",
		des = "触发地震对敌人群体造成999%法术强度+999点地属性伤害,相邻敌人受到伤害会衰减",
		skillPre = 0,
		skillPreLv = 0,
		skillPrePt = 0,
		maxLv = 1,
		attackType = 2,
		dstType = 0,
		singTime = 2.8,
		minSingTime = 1,
		cdTime = 1.5,
		skilleffect = {900002.0},
		buff = {},
		jobID = 10012,
	},

	[108020] = {
		name = " 泥沼地",
		des = "召唤一片泥沼地,使群体敌人攻击速度降低999点,躲闪降低999点,持续999秒",
		skillPre = 0,
		skillPreLv = 0,
		skillPrePt = 0,
		maxLv = 1,
		attackType = 2,
		dstType = 0,
		singTime = 2.8,
		minSingTime = 1,
		cdTime = 1.5,
		skilleffect = {900002.0},
		buff = {},
		jobID = 10012,
	},

	[108030] = {
		name = "火焰之墙",
		des = "给友方单体施放一面火墙,有999%概率击退近战攻击的敌人,并造成999%法术强度+999点火属性伤害.",
		skillPre = 0,
		skillPreLv = 0,
		skillPrePt = 0,
		maxLv = 1,
		attackType = 2,
		dstType = 0,
		singTime = 2.8,
		minSingTime = 1,
		cdTime = 1.5,
		skilleffect = {900002.0},
		buff = {},
		jobID = 10012,
	},

	[108040] = {
		name = "暴风雪",
		des = "召唤暴风雪对敌人全体造成999%法术强度+999点水属性伤害,并有999%概率使敌人冰冻",
		skillPre = 0,
		skillPreLv = 0,
		skillPrePt = 0,
		maxLv = 1,
		attackType = 2,
		dstType = 0,
		singTime = 2.8,
		minSingTime = 1,
		cdTime = 1.5,
		skilleffect = {900002.0},
		buff = {},
		jobID = 10012,
	},

	[108050] = {
		name = "火陨石",
		des = "召唤火陨石对敌人全体造成999%法术强度+999点水属性伤害,并有999%概率使敌人昏迷",
		skillPre = 0,
		skillPreLv = 0,
		skillPrePt = 0,
		maxLv = 1,
		attackType = 2,
		dstType = 0,
		singTime = 2.8,
		minSingTime = 1,
		cdTime = 1.5,
		skilleffect = {900002.0},
		buff = {},
		jobID = 10012,
	},

	[108060] = {
		name = "怒雷强击",
		des = "召唤雷击对敌人全体造成999%法术强度+999点雷属性伤害,并有999%概率使敌人陷入黑暗状态",
		skillPre = 0,
		skillPreLv = 0,
		skillPrePt = 0,
		maxLv = 1,
		attackType = 2,
		dstType = 0,
		singTime = 2.8,
		minSingTime = 1,
		cdTime = 1.5,
		skilleffect = {900002.0},
		buff = {},
		jobID = 10012,
	},

	[109000] = {
		name = "拳刃修练",
		des = "使用拳刃时增加999攻击力",
		skillPre = 0,
		skillPreLv = 0,
		skillPrePt = 0,
		maxLv = 1,
		attackType = 2,
		dstType = 0,
		singTime = 2.8,
		minSingTime = 1,
		cdTime = 1.5,
		skilleffect = {900002.0},
		buff = {},
		jobID = 10014,
	},

	[109010] = {
		name = "短剑修炼",
		des = "使用短剑时增加999攻击力",
		skillPre = 0,
		skillPreLv = 0,
		skillPrePt = 0,
		maxLv = 1,
		attackType = 2,
		dstType = 0,
		singTime = 2.8,
		minSingTime = 1,
		cdTime = 1.5,
		skilleffect = {900002.0},
		buff = {},
		jobID = 10014,
	},

	[109020] = {
		name = " 无影之牙",
		des = "对敌人单体造成999%攻击力+999点物理伤害,潜行时使用不会现形",
		skillPre = 0,
		skillPreLv = 0,
		skillPrePt = 0,
		maxLv = 1,
		attackType = 2,
		dstType = 0,
		singTime = 2.8,
		minSingTime = 1,
		cdTime = 1.5,
		skilleffect = {900002.0},
		buff = {},
		jobID = 10014,
	},

	[109030] = {
		name = " 音速投掷",
		des = "对敌人单体造成999%攻击力+999点物理伤害,并有999%概率使敌人昏迷",
		skillPre = 0,
		skillPreLv = 0,
		skillPrePt = 0,
		maxLv = 1,
		attackType = 2,
		dstType = 0,
		singTime = 2.8,
		minSingTime = 1,
		cdTime = 1.5,
		skilleffect = {900002.0},
		buff = {},
		jobID = 10014,
	},

	[109040] = {
		name = " 毒性感染",
		des = "对敌人群体造成999%攻击力+999点毒伤害,并有999%概率使敌人中毒,相邻敌人受到伤害会衰减",
		skillPre = 0,
		skillPreLv = 0,
		skillPrePt = 0,
		maxLv = 1,
		attackType = 2,
		dstType = 0,
		singTime = 2.8,
		minSingTime = 1,
		cdTime = 1.5,
		skilleffect = {900002.0},
		buff = {},
		jobID = 10014,
	},

	[109050] = {
		name = "涂毒",
		des = "使一个友方单位的武器变为毒属性,物理攻击时有999%使敌人中毒,持续999秒",
		skillPre = 0,
		skillPreLv = 0,
		skillPrePt = 0,
		maxLv = 1,
		attackType = 2,
		dstType = 0,
		singTime = 2.8,
		minSingTime = 1,
		cdTime = 1.5,
		skilleffect = {900002.0},
		buff = {},
		jobID = 10014,
	},

	[109060] = {
		name = "毒性外衣",
		des = "被近战攻击时,对敌人造成999%攻击力+999点毒属性伤害.并免疫毒性伤害",
		skillPre = 0,
		skillPreLv = 0,
		skillPrePt = 0,
		maxLv = 1,
		attackType = 2,
		dstType = 0,
		singTime = 2.8,
		minSingTime = 1,
		cdTime = 1.5,
		skilleffect = {900002.0},
		buff = {},
		jobID = 10014,
	},

	[109070] = {
		name = "潜行",
		des = "进入隐身状态,免疫物理伤害,不能成为技能攻击目标,持续999秒.使用技能后解除.解除状态后的下一次攻击伤害提升999%.",
		skillPre = 0,
		skillPreLv = 0,
		skillPrePt = 0,
		maxLv = 1,
		attackType = 2,
		dstType = 0,
		singTime = 2.8,
		minSingTime = 1,
		cdTime = 1.5,
		skilleffect = {900002.0},
		buff = {},
		jobID = 10014,
	},

	[110000] = {
		name = "驯鹰术",
		des = "召唤猎鹰助战,并增加猎鹰闪电冲击时999点伤害。射击时猎鹰有概率自动攻击,自动释放概率与幸运值相关",
		skillPre = 0,
		skillPreLv = 0,
		skillPrePt = 0,
		maxLv = 1,
		attackType = 2,
		dstType = 0,
		singTime = 2.8,
		minSingTime = 1,
		cdTime = 1.5,
		skilleffect = {900002.0},
		buff = {},
		jobID = 10018,
	},

	[110010] = {
		name = "动物杀手",
		des = "增加自身面对野兽系和龙系敌人时的攻击力999点,防御力999点",
		skillPre = 0,
		skillPreLv = 0,
		skillPrePt = 0,
		maxLv = 1,
		attackType = 2,
		dstType = 0,
		singTime = 2.8,
		minSingTime = 1,
		cdTime = 1.5,
		skilleffect = {900002.0},
		buff = {},
		jobID = 10018,
	},

	[110020] = {
		name = "闪电冲击",
		des = "利用猎鹰对群体敌人造成造成999%攻击力+999点物理伤害",
		skillPre = 0,
		skillPreLv = 0,
		skillPrePt = 0,
		maxLv = 1,
		attackType = 2,
		dstType = 0,
		singTime = 2.8,
		minSingTime = 1,
		cdTime = 1.5,
		skilleffect = {900002.0},
		buff = {},
		jobID = 10018,
	},

	[110030] = {
		name = "猎鹰寻敌",
		des = "利用猎鹰攻击潜行的敌人,对敌人单体造成999%攻击力+999点远程物理伤害",
		skillPre = 0,
		skillPreLv = 0,
		skillPrePt = 0,
		maxLv = 1,
		attackType = 2,
		dstType = 0,
		singTime = 2.8,
		minSingTime = 1,
		cdTime = 1.5,
		skilleffect = {900002.0},
		buff = {},
		jobID = 10018,
	},

	[110040] = {
		name = "地雷陷阱",
		des = "埋下陷阱,队友被近战攻击时触发.造成999%攻击力+999点火属性伤害.每个猎人在同一时间内,只能给队友施放一种陷阱效果.",
		skillPre = 0,
		skillPreLv = 0,
		skillPrePt = 0,
		maxLv = 1,
		attackType = 2,
		dstType = 0,
		singTime = 2.8,
		minSingTime = 1,
		cdTime = 1.5,
		skilleffect = {900002.0},
		buff = {},
		jobID = 10018,
	},

	[110050] = {
		name = "定身陷阱",
		des = "埋下陷阱,队友被近战攻击时触发.造成999%攻击力+999点地伤害,并使敌人被定身.每个猎人在同一时间内,只能给队友施放一种陷阱效果.",
		skillPre = 0,
		skillPreLv = 0,
		skillPrePt = 0,
		maxLv = 1,
		attackType = 2,
		dstType = 0,
		singTime = 2.8,
		minSingTime = 1,
		cdTime = 1.5,
		skilleffect = {900002.0},
		buff = {},
		jobID = 10018,
	},

	[110060] = {
		name = "强光陷阱",
		des = "埋下陷阱,队友被近战攻击时触发.造成999%攻击力+999点雷属性伤害,并使敌人陷入黑暗.每个猎人在同一时间内,只能给队友施放一种陷阱效果.",
		skillPre = 0,
		skillPreLv = 0,
		skillPrePt = 0,
		maxLv = 1,
		attackType = 2,
		dstType = 0,
		singTime = 2.8,
		minSingTime = 1,
		cdTime = 1.5,
		skilleffect = {900002.0},
		buff = {},
		jobID = 10018,
	},

	[110070] = {
		name = "霜冻陷阱",
		des = "埋下陷阱,队友被近战攻击时触发.造成999%攻击力+999点水属性伤害,并使敌人被冰冻.每个猎人在同一时间内,只能给队友施放一种陷阱效果.",
		skillPre = 0,
		skillPreLv = 0,
		skillPrePt = 0,
		maxLv = 1,
		attackType = 2,
		dstType = 0,
		singTime = 2.8,
		minSingTime = 1,
		cdTime = 1.5,
		skilleffect = {900002.0},
		buff = {},
		jobID = 10018,
	},

	[110080] = {
		name = "魔耗陷阱",
		des = "埋下陷阱,队友被近战攻击时触发.对敌人造成999点魔法值伤害.每个猎人在同一时间内,只能给队友施放一种陷阱效果.",
		skillPre = 0,
		skillPreLv = 0,
		skillPrePt = 0,
		maxLv = 1,
		attackType = 2,
		dstType = 0,
		singTime = 2.8,
		minSingTime = 1,
		cdTime = 1.5,
		skilleffect = {900002.0},
		buff = {},
		jobID = 10018,
	},

	[110090] = {
		name = "陷阱移除",
		des = "让猎鹰随机拆除敌人的一个陷阱",
		skillPre = 0,
		skillPreLv = 0,
		skillPrePt = 0,
		maxLv = 1,
		attackType = 2,
		dstType = 0,
		singTime = 2.8,
		minSingTime = 1,
		cdTime = 1.5,
		skilleffect = {900002.0},
		buff = {},
		jobID = 10018,
	},

}

