local function test(pid)
	...
	assert(task)
	net.task.C2S.executetask(player,{ taskid = 10001,npcid = 1001})
	...
	task = player.taskdb:gettask(10001)
	assert(task)
	net.task.C2S.finishtask(player,{taskid = 10001,npcid = 1001})
	task = player.taskdb:gettask(10001)
	assert(not task)
	--寻物任务,服务端执行,自动提交
	...
	net.task.C2S.executetask(player,{taskid = 10002,npcid = 1002,ext = {{itemid=itemid,num=1},}})
	...
	assert(task.state)
	net.task.C2S.executetask(player,{taskid = 10003,npcid = 1003})
	assert(task.state == 1)
